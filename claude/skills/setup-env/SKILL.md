...
- [ ] (선택) `~/.zsh_history` 마이그레이션 — atuin 권장, docs/migration.md 참조
- [ ] GPG 키 없으면 `~/.gitconfig_local` 에 `[commit] gpgsign = false` 추가
- [ ] 새 셸 시작: `exec zsh`
```

`docs/setup.md` (셋업 작업) 와 `docs/migration.md` (마이그레이션 항목) 가 단일 진실 출처이므로 **새 항목을 임의로 추가하지 말고**, 항목이 늘면 해당 문서에 먼저 기록할 것을 사용자에게 권한다.
