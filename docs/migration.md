...
> `~/.gitconfig` 자체는 본 dotfiles 의 `git/gitconfig` 로 심링크되므로 옮기지 말 것. 호스트별 user.email 은 `~/.gitconfig_local` 에 작성합니다 ([setup.md 의 Git 섹션](setup.md#git) 참조).
...
| `~/.aws/credentials` | **절대 금지** | 1Password CLI (`op`) 로 관리 — [secrets.md](secrets.md) |
...
1Password SSH agent 사용 시 키는 1Password 가 관리합니다. dotfiles 에는 `~/.ssh/config` 만 두면 충분.
